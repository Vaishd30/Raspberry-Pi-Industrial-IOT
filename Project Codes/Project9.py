...
def measure(channel):
    ...
    try:
        for _ in range(5):
            ...
            AirDensity = Pressure/(287.058*(Temperature+273.15))
            print(f'Date={date} Time={time} Temp={Temperature:0.1f}*C Humidity={Humidity:0.1f}% Pressure={Pressure} Pa Altitude={Altitude:0.1f} m SeaLevelPressure={SeaLevelPressure} Pa AmbientLight={AmbientLight:0.1f} Lux DewPoint={DewPoint}*C Air Density={AirDensity:0.1f} kg/cubicmeter')
            # Append data as a dictionary
            data_list.append({'Date': date, 'Time': time, 'Temperature': Temperature, 'Humidity': Humidity,'Pressure':Pressure , 'Altitude':Altitude, 'SeaLevelPressure':SeaLevelPressure,'AmbientLight':AmbientLight, 'DewPoint':DewPoint,'AirDensity':AirDensity})
            
            sleep(2)  # Delay between updates
            
        # Write to ThingSpeak
            response = channel.update({1: Temperature, 2: Humidity,3: Pressure , 4:Altitude, 5:SeaLevelPressure, 6: AmbientLight, 7:DewPoint, 8:AirDensity})
        ...
        with open("sensor_data.csv", "a", newline="") as file:
            fieldnames = ['Date', 'Time', 'Temperature', 'Humidity','Pressure','Altitude','SeaLevelPressure','AmbientLight','DewPoint','AirDensity']
